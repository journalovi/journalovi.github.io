...
Such self-contained web applications will likely remain working in the long run. It may be possible to make your software self-contained as a WebAssembly application. See ongoing work in [R](https://posit-dev.github.io/r-shinylive/) and [Python](https://github.com/posit-dev/py-shinylive) 
However, there could be situations where the solutions above do not work or are unsuitable, e.g., works that require heavy computation. If server-side execution is unavoidable, we expect the following in your submission:
