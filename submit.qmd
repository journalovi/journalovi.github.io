...
We strongly recommend papers on the "Github" track be written in [Quarto](https://quarto.org/), a markdown-based computational notebook format. Quarto is a plain text format that can be used with documents written in a variety of forms (RMarkdown, Jupyter notebook, Word, Latex), and supports embedded computation (in many languages, including R, python, and Julia) and interactivity (using JavaScript / Observable, d3, etc). In keeping with the experimental nature of this track, we will also consider other formats, so long as the output contains the same required information as the Quarto template and is rendered in HTML --- if you are interested in trying another format, please email [submit@journalovi.org](mailto:submit@journalovi.org).
